...
public final class OwnerDamageSource {
    ...
    public static boolean isCurrentOwnerDamage(EntityMaid maid, DamageSource source) {
        if (!isEnabled() || (maid.getOwner() == null ? null : maid.getOwner().getUUID()) == null || source == null) {
            return false;
        }
    ...
    public static boolean isCurrentOwnerSource(EntityMaid maid, DamageSource source) {
        if ((maid.getOwner() == null ? null : maid.getOwner().getUUID()) == null || source == null) {
            return false;
        }
        UUID ownerId = (maid.getOwner() == null ? null : maid.getOwner().getUUID());
        return belongsToPlayer(source.getEntity(), ownerId, 0, new HashSet<>())
    ...
    public static boolean belongsToOwnerDamageEvent(Object event) {
        ...
        for (EntityMaid target : targets) {
            ...
            UUID ownerId = (target.getOwner() == null ? null : target.getOwner().getUUID());
            if (ownerId == null) {
                continue;
            }
    ...
    public static boolean isCurrentOwnerEntity(EntityMaid maid, @Nullable Entity sourceEntity) {
        if (!isEnabled() || (maid.getOwner() == null ? null : maid.getOwner().getUUID()) == null) {
            return false;
        }
        return belongsToPlayer(sourceEntity, (maid.getOwner() == null ? null : maid.getOwner().getUUID()), 0, new HashSet<>());
    }
