...
import com.github.JumDa5he.callresponse.network.HuntOrderUpdateC2SPacket;
...
public class HuntOrderScreen extends Screen {
    ...
    private void onAdd() {
        ...
        input.setValue("");
        ClientPacketDistributor.sendToServer(new HuntOrderUpdateC2SPacket(maidUUID, false, value));
    }
    ...
    private void onRemove(UUID uuid) {
        ...
        ClientPacketDistributor.sendToServer(new HuntOrderUpdateC2SPacket(maidUUID, true, uuid.toString()));
    ...
    public boolean keyPressed(KeyEvent event) {
        if ((event.key() == 257 || event.key() == 335) && input != null && input.isFocused()) {
            onAdd();
            return true;
        }
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.item(new ItemStack(ModItems.HUNT_ORDER.get()), left + 14, top + 11);
        graphics.text(font, getTitle(), left + 38, top + 12, 0xFFF2D18A, false);
        graphics.text(font, Component.translatable("gui.callresponse.hunt.maid", shortUuid(maidUUID)),
        ...
        graphics.text(font, count, right - 16 - font.width(count), top + 18, 0xFFD66A6A, false);

        int start = page * visibleRows;
        int end = Math.min(start + visibleRows, entries.size());
        if (entries.isEmpty()) {
            Component empty = Component.translatable("gui.callresponse.hunt.empty");
            graphics.centeredText(font, empty, width / 2, listTop() + 17, 0xFF858585);
        }
        for (int i = start; i < end; i++) {
            HuntOrderEntry entry = entries.get(i);
            ...
            graphics.text(font, type, left + 23, y + 6, typeColor, false);
            int nameX = left + 23 + font.width(type) + 6;
            int maxNameWidth = right - 62 - nameX;
            String label = entry.name + "  §8#" + shortUuid(entry.uuid);
            graphics.text(font, font.plainSubstrByWidth(label, Math.max(20, maxNameWidth)),
                    nameX, y + 6, 0xFFE6E6E6, false);
        }

        Component pageText = Component.translatable("gui.callresponse.hunt.page", page + 1, maxPage());
        graphics.centeredText(font, pageText, width / 2, footerY() + 6, 0xFFA7A7A7);
    }

    private static void drawPanel(GuiGraphicsExtractor graphics, int left, int top, int right, int bottom) {
        graphics.fill(left, top, right, bottom, 0xE8110E12);
        graphics.fill(left + 1, top + 1, right - 1, top + 42, 0xE52B1219);
        graphics.fill(left, top, right - 1, top + 1, 0xFFD75A54);
        graphics.fill(left, bottom - 1, right - 1, bottom - 1 + 1, 0xFF6E3437);
        graphics.fill(left, top, left + 1, bottom - 1, 0xFF6E3437);
        graphics.fill(right - 1, top, right - 1 + 1, bottom - 1, 0xFF6E3437);
        graphics.fill(left + 10, top + 42, right - 11, top + 42 + 1, 0xFF4C272D);
    }
