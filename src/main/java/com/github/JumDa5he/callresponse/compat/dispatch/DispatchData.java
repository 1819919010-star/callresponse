...
public final class DispatchData extends SavedData {
    ...
    public static DispatchData get(MinecraftServer server) {
        ...
        return overworld.getDataStorage().computeIfAbsent(
                new SavedData.Factory<>(DispatchData::new, DispatchData::load, null),
                DATA_NAME);
    }

    public static DispatchData load(CompoundTag tag, HolderLookup.Provider provider) {
        DispatchData data = new DispatchData();
        ListTag records = tag.getList("Records", Tag.TAG_COMPOUND);
        for (int i = 0; i < records.size(); i++) {
            DispatchRecord record = DispatchRecord.load(records.getCompound(i), provider);
    ...
    public CompoundTag save(CompoundTag tag, HolderLookup.Provider provider) {
        ...
        records.values().forEach(list -> list.forEach(record -> recordList.add(record.save(provider))));
    ...
    public record DispatchRecord(UUID dispatchId, UUID ownerId, UUID maidId, CompoundTag maidNbt,
        ...
        public static DispatchRecord load(CompoundTag tag, HolderLookup.Provider provider) {
            List<ItemStack> rewards = new ArrayList<>();
            ListTag list = tag.getList("Rewards", Tag.TAG_COMPOUND);
            for (int i = 0; i < list.size(); i++) {
                rewards.add(ItemStack.parse(provider, list.getCompound(i)).orElse(ItemStack.EMPTY));
            }
