...
import com.github.JumDa5he.callresponse.network.WanderingMaidDecisionC2SPacket;
...
public final class WanderingMaidRequestScreen extends Screen {
    ...
    private WanderingMaidRequestScreen(int entityId, UUID maidId, Component maidName) {
        super(Component.translatable("gui.callresponse.wandering.request.title"));
    ...
    protected void init() {
        ...
        addRenderableWidget(Button.builder(Component.translatable("gui.callresponse.wandering.request.accept"),
                        button -> decide(true))
                .pos(centerX - 112, buttonY).size(104, 22).build());
        addRenderableWidget(Button.builder(Component.translatable("gui.callresponse.wandering.request.reject"),
                        button -> decide(false))
                .pos(centerX + 8, buttonY).size(104, 22).build());
    }

    private void decide(boolean accept) {
        ClientPacketDistributor.sendToServer(new WanderingMaidDecisionC2SPacket(maidId, accept));
        onClose();
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        // graphics.renderOutline(panelLeft, panelTop, 316, 200, 0xFFD6A75C); // 26.1 removed
        // graphics.renderOutline(panelLeft + 4, panelTop + 4, 308, 192, 0xFF754E68); // 26.1 removed
        // graphics.renderOutline(panelLeft + 12, panelTop + 43, 98, 112, 0xFF8F6D83); // 26.1 removed
        drawCornerOrnaments(graphics, panelLeft, panelTop, panelRight, panelBottom);

        graphics.pose().pushMatrix();
        graphics.pose().translate(centerX, panelTop + 8).scale(1.35f, 1.35f);
        graphics.centeredText(font, title, 0, 0, 0xFFFFE9B9);
        graphics.pose().popMatrix();
        graphics.centeredText(font,
                Component.translatable("gui.callresponse.wandering.request.subtitle"),
        ...
        if (entity instanceof EntityMaid maid) {
            EntityRenderUtil.renderMaid(graphics, maid, centerX - 97, panelTop + 100, 28, 0);
        }
        ...
        Component description = Component.translatable("gui.callresponse.wandering.request.description", shownName);
        graphics.text(font, Component.translatable("gui.callresponse.wandering.request.name", shownName),
        ...
        for (FormattedCharSequence line : lines) {
            graphics.text(font, line, centerX - 38, textY, 0xFFE8E1E7, false);
            textY += 13;
        }
        graphics.text(font, Component.translatable("gui.callresponse.wandering.request.hint"),
                centerX - 38, panelBottom - 52, 0xFF9E929C, false);

        super.extractRenderState(graphics, mouseX, mouseY, partialTick);
    }

    private static void drawCornerOrnaments(GuiGraphicsExtractor graphics, int left, int top, int right, int bottom) {
        int gold = 0xFFD6A75C;
        int rose = 0xFF8F5877;
