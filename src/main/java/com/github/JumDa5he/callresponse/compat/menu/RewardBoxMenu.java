...
public final class RewardBoxMenu extends AbstractContainerMenu {
    ...
    public RewardBoxMenu(int id, Inventory inventory) {
        this(id, inventory, new SimpleContainer(RewardBoxBlockEntity.SIZE));
    }
    ...
    public RewardBoxMenu(int id, Inventory inventory, Container container) {
        ...
        for (int row = 0; row < 9; row++) {
            for (int col = 0; col < 12; col++) {
                addSlot(new Slot(container, col + row * 12, 8 + col * 18, 18 + row * 18));
            }
        }
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                addSlot(new Slot(inventory, col + row * 9 + 9, 35 + col * 18, 184 + row * 18));
            }
        }
        for (int col = 0; col < 9; col++) {
            addSlot(new Slot(inventory, col, 35 + col * 18, 242));
        }
    }
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (index < RewardBoxBlockEntity.SIZE) {
            if (!moveItemStackTo(moving, RewardBoxBlockEntity.SIZE, slots.size(), true)) {
                return ItemStack.EMPTY;
            }
        } else if (!moveItemStackTo(moving, 0, RewardBoxBlockEntity.SIZE, false)) {
            return ItemStack.EMPTY;
        }
        if (moving.isEmpty()) {
