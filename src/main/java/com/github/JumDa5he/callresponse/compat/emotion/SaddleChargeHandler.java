...
public class SaddleChargeHandler {
    ...
    public static void onMousePress(InputEvent.MouseButton.Pre event){
        ...
        if(event.getAction() == 0){
            ...
            ClientPacketDistributor.sendToServer(new DropMaidC2SPacket(chargePercent));
        } else {
            chargePercent = 0;
            pressed = true;
        }
    ...
    public static void onRenderGui(RenderGuiEvent.Post event) {
        ...
        if (fillWidth > 0) {
            GuiGraphicsExtractor.fill(x, y, x + fillWidth, y + barHeight, 0xFF00FF00);
        }
        // GuiGraphicsExtractor.renderOutline(x, y, barWidth, barHeight, 0xFFFFFFFF); // 26.1 removed
    }
}
