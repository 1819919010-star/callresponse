...
public final class NpcEventScreen extends Screen {
    ...
    private void choose(int index) {
        ClientPacketDistributor.sendToServer(new NpcEventChoiceC2SPacket(maidId, index));
        onClose();
    }
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.outline(left, top, 328, 210, 0xFFE0B76A);
        graphics.outline(left + 4, top + 4, 320, 202, 0xFF805D79);

        graphics.pose().pushMatrix();
        graphics.pose().translate(centerX, top + 10);
        graphics.pose().scale(1.25F, 1.25F);
        graphics.centeredText(font, Component.translatable(titleKey), 0, 0, 0xFFFFE8B5);
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (entity instanceof EntityMaid maid) {
            EntityRenderUtil.renderMaid(graphics, maid, left + 67, top + 147, 42, 0);
            graphics.centeredText(font, maid.getDisplayName(), left + 67,
                    top + 181, 0xFFE8DCE7);
        }
        ...
        for (FormattedCharSequence line : lines) {
            graphics.text(font, line, left + 139, textY, 0xFFE9E1E8, false);
            textY += 12;
            if (textY > top + 82) break;
        }
        graphics.text(font, Component.translatable("gui.callresponse.npc_event.choose"),
