...
public class CallResponseJadePlugin implements IWailaPlugin{
    ...
    public static void addJadeInfo(AddJadeInfoEvent event){
        if(Screen.hasShiftDown()){
            var mood = EmotionData.get(event.getMaid(), Minecraft.getInstance().getGameProfile().getId());
            ...
            var name = EmotionData.getTendency(event.getMaid(), Minecraft.getInstance().getGameProfile().getId()).getName();
