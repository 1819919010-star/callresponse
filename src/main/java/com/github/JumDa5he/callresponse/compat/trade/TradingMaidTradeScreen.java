...
public final class TradingMaidTradeScreen extends Screen {
    ...
    private TradingMaidTradeScreen(int traderId,
        ...
        super(Component.translatable("gui.callresponse.trade.title"));
    ...
    private void addColumnButtons(int left, int top, boolean buying) {
        ...
        Component label = Component.translatable(buying ? "gui.callresponse.trade.buy" : "gui.callresponse.trade.sell");
    ...
    private void act(boolean buying) {
        ...
        ClientPacketDistributor.sendToServer(new TradingMaidActionC2SPacket(traderId, list.get(index).uuid(),
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        // graphics.renderOutline(left, top, 180, 200, 0xFF72B8A9); // 26.1 removed
        // graphics.renderOutline(left + 184, top, 180, 200, 0xFFD18A70); // 26.1 removed
        graphics.centeredText(font, Component.translatable("gui.callresponse.trade.buy_side"),
                left + 90, top + 9, 0xFFFFFFFF);
        graphics.centeredText(font, Component.translatable("gui.callresponse.trade.sell_side"),
    ...
    private void renderEntry(GuiGraphicsExtractor graphics, int left, int top,
                             List<OpenTradingMaidScreenS2CPacket.MaidInfo> list, int index,
                             int mouseX, int mouseY, boolean buying) {
        if (list.isEmpty()) {
            graphics.centeredText(font, Component.translatable(buying
                    ? "gui.callresponse.trade.empty_buy" : "gui.callresponse.trade.empty_sell"),
                    left + 90, top + 88, 0xFFA9A9A9);
            return;
        }
        OpenTradingMaidScreenS2CPacket.MaidInfo info = list.get(Math.min(index, list.size() - 1));
        Entity entity = minecraft != null && minecraft.level != null ? minecraft.level.getEntity(info.entityId()) : null;
        if (entity instanceof EntityMaid maid) {
            EntityRenderUtil.renderMaid(graphics, maid, left + 90, top + 80, 20, 0);
        }
        ...
        graphics.centeredText(font, modelName, left + 90, top + 129, 0xFFD8D8D8);
        if (info.customNamed()) {
            graphics.pose().pushMatrix();
            graphics.pose().translate(left + 90, top + 31).scale(1.2F, 1.2F);
            graphics.centeredText(font, info.displayName(), 0, 0, 0xFFFFD36A);
            graphics.pose().popMatrix();
        } else {
            graphics.centeredText(font, info.displayName(), left + 90, top + 33, 0xFFFFFFFF);
        }
        if (buying) {
            graphics.centeredText(font,
                    Component.translatable("gui.callresponse.trade.price", info.emeraldValue()),
                    left + 90, top + 143, 0xFF69D67C);
        } else {
            graphics.centeredText(font, Component.translatable("gui.callresponse.trade.sale_reward", info.emeraldValue()),
                    left + 90, top + 143, 0xFFE2BA73);
        }
        graphics.centeredText(font, Component.literal((Math.min(index, list.size() - 1) + 1) + " / " + list.size()),
