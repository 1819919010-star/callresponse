...
import com.github.JumDa5he.callresponse.network.RequestHuntOrderScreenC2SPacket;
...
public class MaidListScreen extends Screen {
    ...
    private void select(UUID maidUuid) {
        ClientPacketDistributor.sendToServer(new RequestHuntOrderScreenC2SPacket(maidUuid));
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(font, subtitle, left + 38, top + 27, 0xFF9A9A9A, false);

        int start = page * visibleRows;
        int end = Math.min(start + visibleRows, maids.size());
        if (maids.isEmpty()) {
            Component empty = Component.translatable("gui.callresponse.hunt.terminal.empty");
            graphics.centeredText(font, empty, width / 2, listTop() + 22, 0xFFD16A6A);
            Component hint = Component.translatable("gui.callresponse.hunt.terminal.empty.hint");
            graphics.centeredText(font, hint, width / 2, listTop() + 38, 0xFF888888);
        }
        ...
        for (int i = start; i < end; i++) {
            ...
            String name = info.customName == null || info.customName.isBlank()
                    ...
                    : info.customName;
            graphics.text(font, font.plainSubstrByWidth(name, panelWidth() - 145),
                    left + 23, y + 4, 0xFFF0E2C7, false);
            String detail = "#" + shortUuid(info.uuid) + "  " + Component.translatable(info.typeKey).getString();
            graphics.text(font, font.plainSubstrByWidth(detail, panelWidth() - 145),
    ...
    private static void drawPanel(GuiGraphicsExtractor graphics, int left, int top, int right, int bottom) {
        ...
        graphics.fill(left + 10, top + 43, right - 11, top + 43 + 1, 0xFF4C272D);
    }
