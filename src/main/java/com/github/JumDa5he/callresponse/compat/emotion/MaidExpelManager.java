...
public final class MaidExpelManager {
    ...
    public static void frighten(ServerPlayer player, EntityMaid maid) {
        ...
        long until = InitAttachTypes.persistentData(maid).getLong(FRIGHTEN_UNTIL_TAG).orElse(0L);
        if (now < until) {
            player.sendSystemMessage(Component.literal("今天已经用过‘吓吓你的’了。"));
            return;
        }
        InitAttachTypes.persistentData(maid).putLong(FRIGHTEN_UNTIL_TAG, now + ONE_GAME_DAY);
