...
public final class WanderingMaidData {
    /** Synced to the client so client-side mobInteract cannot simulate cake taming. */
    public static final EntityDataAccessor<Boolean> SPECIAL_SYNC =
    ...
    public static void initialize(EntityMaid maid, UUID target, long gameTime) {
        ...
        maid.getEntityData().set(SPECIAL_SYNC, true);
    ...
    public static boolean isSpecial(EntityMaid maid) {
        return maid.getEntityData().get(SPECIAL_SYNC)
                || maid.getPersistentData().getBoolean(SPECIAL);
    }

    public static void restoreSyncedFlag(EntityMaid maid) {
        maid.getEntityData().set(SPECIAL_SYNC, maid.getPersistentData().getBoolean(SPECIAL));
    }
    ...
    public static void clear(EntityMaid maid) {
        CompoundTag tag = maid.getPersistentData();
        maid.getEntityData().set(SPECIAL_SYNC, false);
        tag.remove(SPECIAL);
        tag.remove(STATE);
        tag.remove(TARGET);
        tag.remove(ARRIVALS);
