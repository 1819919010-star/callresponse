...
public class EmotionDotingManager {
    ...
    private static class ActionTask {
        ...
        BlockPos target;
        ServerPlayer player;
        ActionTask(ActionType type, BlockPos target, ServerPlayer player) {
            this.type = type;
            this.target = target;
            this.player = player;
        }
        ...
        for (int dx = -radius; dx <= radius; dx++) {
            for (int dz = -radius; dz <= radius; dz++) {
                for (int dy = -3; dy <= 3; dy++) {
                    ...
                    BlockState state = maid.level().getBlockState(checkPos);
    ...
    private static boolean startAction(EntityMaid maid, ServerPlayer player, ActionType type) {
        ...
        switch (type) {
            case STEAL:
                if (player == null) return false;
                pendingTasks.put(maid.getUUID(), new ActionTask(type, null, player));
                walkTo(maid, player.blockPosition());
                return true;
            case SEARCH:
                BlockPos container = findNearestContainer(maid);
                if (container == null) return false;
                pendingTasks.put(maid.getUUID(), new ActionTask(type, container, player));
    ...
    private static void executeSearch(EntityMaid maid, ServerPlayer player, BlockPos targetPos) {
        ...
        if (be == null) return;
        IItemHandler handler = be.getCapability(net.minecraftforge.common.capabilities.ForgeCapabilities.ITEM_HANDLER)
                .orElse(null);
